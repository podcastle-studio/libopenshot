...
static inline float denoise_lerp(float a, float b, float amount) {
	return a + ((b - a) * amount);
}
	...
	for (int y = 0; y < height; ++y) {
		for (int x = 0; x < width; ++x) {
			...
			if (temporal_valid) {
				...
				out_y = denoise_lerp(out_y, prev_y, temporal_luma_mix);
				out_r = out_y + denoise_lerp(out_chroma_r, prev_r - prev_y, temporal_chroma_mix);
				out_g = out_y + denoise_lerp(out_chroma_g, prev_g - prev_y, temporal_chroma_mix);
				out_b = out_y + denoise_lerp(out_chroma_b, prev_b - prev_y, temporal_chroma_mix);
			}
