...
static inline float tape_lerp(float a, float b, float t) { return a + (b - a) * t; }
...
#pragma omp parallel for
	...
	int r_y = std::round(tape_lerp(0.0f, 2.0f, k_soft) * inverse_scale_x);
	if (k_noise > 0.6f)
		r_y = std::min(r_y, 1);
	if (r_y > 0) {
#ifdef _OPENMP
#pragma omp parallel for
	...
	float shift = tape_lerp(0.0f, 2.5f, k_bleed) * inverse_scale_x;
	int r_c = std::round(tape_lerp(0.0f, 3.0f, k_bleed) * inverse_scale_x);
...
#pragma omp parallel for
	...
	float sigmaY = tape_lerp(0.0f, 0.08f, k_noise);
	const float decay = 0.88f + 0.08f * k_noise;
	const float amp = 0.18f * k_noise;
	const float baseP = 0.0025f + 0.02f * k_noise;

	float Hfixed = tape_lerp(0.0f, 0.12f * h, k_stripe);
	float Gfixed = 0.10f * k_stripe;
	float Nfixed = 1.0f + 1.5f * k_stripe;

	float rate = 0.4f * k_bands;
	int dur_frames = std::round(tape_lerp(1.0f, 6.0f, k_bands));
	float Hburst = tape_lerp(0.06f * h, 0.25f * h, k_bands);
	float Gburst = tape_lerp(0.10f, 0.25f, k_bands);
	float sat_band = tape_lerp(0.8f, 0.5f, k_bands);
...
#pragma omp parallel for
	...
	float A = tape_lerp(0.0f, 3.0f, k_track) * inverse_scale_x; // pixels
	float f = tape_lerp(0.25f, 1.2f, k_track); // Hz
	float Hsk = tape_lerp(0.0f, 0.10f * h, k_track); // pixels
	float S = tape_lerp(0.0f, 5.0f, k_track) * inverse_scale_x; // pixels
