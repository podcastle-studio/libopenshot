...
static float grain_lerp(float a, float b, float t) {
	return a + ((b - a) * t);
}
...
std::shared_ptr<openshot::Frame> FilmGrain::GetFrame(std::shared_ptr<openshot::Frame> frame, int64_t frame_number) {
	...
	const float cell_size       = grain_lerp(1.0f, 9.0f, size_value);
	const float fine_frequency  = 1.0f / cell_size;
	const float soft_frequency  = fine_frequency * grain_lerp(0.45f, 0.12f, softness_value);
	const float clump_frequency = fine_frequency * grain_lerp(0.35f, 0.08f, clump_value);
	const float temporal_rate   = grain_lerp(0.0f, 1.0f, evolution_value) * grain_lerp(1.0f, 8.0f, 1.0f - coherence_value);
	...
	for (int y = 0; y < height; ++y) {
		...
		// Per pixel we only need one grain_lerp across x instead of 4 hash calls + 3 lerps.
		...
		for (int x = 0; x < width; ++x) {
			...
			// then 1 grain_lerp per pixel instead of 4 hashes + 3 lerps every pixel.
			float soft_sx = 0.0f;
			if (use_softness) {
			...
			for (int s = 0; s < num_salts; ++s) {
				...
				if (use_clump) {
					...
					grain *= grain_lerp(1.0f, 0.45f + (std::abs(cluster) * 1.35f), clump_value);
				}
				grains[s] = grain;
			}
