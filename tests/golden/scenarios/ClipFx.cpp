...
void golden::registerClipFxScenarios() {
    add("clipfx.shadow", {"clipfx", "shadow"}, {1, 45, 89},
        [](Scene& s) {
    ...
    add("clipfx.shadow_colored_sharp", {"clipfx", "shadow"}, {30},
        [](Scene& s) {
            auto& tl = s.makeTimeline();
            tl.AddClip(backgroundClip(s, s.media("background_960x540.png")));
            auto* c = imageClip(s);
    ...
    add("clipfx.blur", {"clipfx", "blur"}, {1, 45, 89},
        [](Scene& s) {
            auto& tl = s.makeTimeline();
            tl.AddClip(backgroundClip(s, s.media("background_960x540.png")));
    ...
    add("clipfx.shadow_blur_rotated", {"clipfx", "shadow", "blur"}, {30},
        [](Scene& s) {
            auto& tl = s.makeTimeline();
            tl.AddClip(backgroundClip(s, s.media("background_960x540.png")));
