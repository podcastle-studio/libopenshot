...
1. Read `STATUS.md` (state, open decisions, next step) and the "Read first" section of `CLAUDE.md`.
...
5. Summarise in at most eight lines: where we are, what the next step is (from `STATUS.md` and the
   matching step in `doc/GPU-RENDER-PLAN.md` with its numeric gate), and anything that looks stale
